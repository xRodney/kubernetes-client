...
public abstract class AbstractJsonSchema<T, B> {
  ...
  public static final String ANNOTATION_SCHEMA_UNROLL = "io.fabric8.crd.generator.annotation.SchemaUnroll";
  ...
  private static SchemaGenerationContext.SchemaUnrollModel extractSchemaUnroll(Object annotation) {
    if (annotation == null) {
      return NO_UNROLL;
    } else if (annotation instanceof SchemaUnroll) {
      SchemaUnroll schemaUnroll = (SchemaUnroll) annotation;
  ...
  private T internalFromImpl(TypeDef definition, SchemaGenerationContext context, String... ignore) {
    ...
    for (Property property : definition.getProperties()) {
      ...
      Property possiblyTerminatedProperty = context.pushLevel(possiblyRenamedProperty);
      final T schema = internalFromImpl(name, possiblyTerminatedProperty.getTypeRef(), context);
      context.popLevel();
      ...
      annotations.forEach(a -> {
        switch (a.getClassRef().getFullyQualifiedName()) {
          ...
          case ANNOTATION_SCHEMA_FROM:
            ...
            break;
          case ANNOTATION_SCHEMA_UNROLL:
            unroll = extractSchemaUnroll(a);
        }
      });
    }
  ...
  private T internalFromImpl(String name, TypeRef typeRef, SchemaGenerationContext context) {
    ...
    if (typeRef.getDimensions() > 0 || io.sundr.model.utils.Collections.isCollection(typeRef)) { // Handle Collections & Arrays
      final TypeRef collectionType = TypeAs.combine(TypeAs.UNWRAP_ARRAY_OF, TypeAs.UNWRAP_COLLECTION_OF)
          .apply(typeRef);
      final T schema = internalFromImpl(name, collectionType, context);
      ...
      if (typeName != null) { // we have a type that we handle specifically
        ...
        if (typeRef instanceof ClassRef) { // Handle complex types
          ...
          if (def.isEnum()) {
            ...
            return internalFromImpl(def, context);
          }
